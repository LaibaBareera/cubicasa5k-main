...
def polygons_to_tensor(polygons_val, types_val, room_polygons_val, room_types_val, size, split=[12]):
    """Room (wall) segmentation only: rasterize room polygons to a 12-channel tensor."""
    ten = np.zeros((split[0], size[0], size[1]))
...
def get_evaluation_tensors(val, model, split, logger, rotate=True, n_classes=44, device=None):
    if device is None:
    ...
    heatmaps_val, rooms_val = post_prosessing.split_validation(
        labels_val, img_size, split)
    heatmaps, rooms = post_prosessing.split_prediction(
    ...
    polygons, types, room_polygons, room_types = post_prosessing.get_polygons(
        (heatmaps, rooms), 0.4, all_opening_types)
    logger.info("Prediction post processing done")

    predicted_classes = polygons_to_tensor(
        polygons, types, room_polygons, room_types, img_size, split=[12])
    
    pol_rooms = np.argmax(predicted_classes, axis=0)

    # Room (wall) segmentation only
    room_gt = labels_val[0, 21].data.numpy()  # [H, W]
    return (np.expand_dims(room_gt, 0), np.array([rooms_seg]), np.array([pol_rooms]))
