...
class DictToTensor(object):
    ...
    def cubi(self, sample):
        ...
        label = torch.cat((heatmap_tensor, label[0:1]), 0)  # label[0]=room, drop icon channel
...
class RandomCropToSizeTorch(object):
    def __init__(self, input_slice=[21, 1], size=(256, 256), fill=(0, 0), data_format='tensor',
    ...
    def augment_tesor(self, sample):
        ...
        label = torch.cat((new_heatmaps, new_rooms), 0)
        image = new_image
...
class ResizePaddedTorch(object):
    ...
    def augment_tensor(self, sample):
        ...
        if self.both:
            ...
            label = torch.cat((heatmaps, rooms_padded), dim=0)
    ...
    def augment_dict_furu(self, sample):
        ...
        _, ratio, y_pad, x_pad = self.resize_padded(label[[1]], self.size, mode='nearest', fill_cval=self.fill[1])
        label = rooms_padded
