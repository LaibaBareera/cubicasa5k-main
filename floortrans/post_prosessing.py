...
def get_polygons(predictions, threshold, all_opening_types):
    if len(predictions) == 3:
        ...
        heatmaps, room_seg = predictions
        # Dummy icon segmentation (no icon prediction) for wall-only pipeline
        height, width = room_seg.shape[1], room_seg.shape[2]
        icon_seg = np.zeros((11, height, width), dtype=np.float32)
    height = room_seg.shape[1]
    width = room_seg.shape[2]
    ...
    openings, opening_types = get_opening_polygon(heatmaps, walls, wall_points, wall_lines, wall_point_orientation_lines_map, threshold, point_orientations, orientation_ranges, all_opening_types, icons_seg=icon_seg)
    ...
    polygons = np.concatenate([walls, openings])
    types = wall_types + opening_types
...
def get_opening_polygon(heatmaps, wall_polygons, wall_points, wall_lines, wall_point_orientation_lines_map, threshold, point_orientations, orientation_ranges, all_opening_types, max_num_points=100, gap=10, icons_seg=None):
    height, width = heatmaps.shape[1], heatmaps.shape[2]
    size = height, width
    if icons_seg is None:
        icons_seg = np.zeros((11, height, width), dtype=np.float32)
...
def extract_wall_polygon(wall, wall_points, segmentation, seg_class):
    ...
    if w_dim == 1:
        ...
        # if len(widths) == 0:
            # return None
        m = stats.mode(widths, keepdims=True)
        wall_width = int(m.mode[0])
        if wall_width > y2 - y1:
            wall_width = y2 - y1
...
def split_validation(tensor, shape, split):
    height = shape[0]
    width = shape[1]
    # Labels: 21 heatmaps + 1 room (no icon)
    heatmaps, rooms = torch.split(tensor, [split[0], 1], 1)
